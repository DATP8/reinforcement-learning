...
class Trainer():
    def __init__(self, model: PVModel, topology: list[tuple[int, int]]):
        ...
        assert self.n_actions == len(topology), "Number of actions must match the number of edges in the topology."
        
    
    def train(self, num_episodes=1000, num_simulations=100, exploration_factor=1.0, mcts_cache_size=1000):
        ...
        for episode in range(num_episodes):
            ...
            for _ in range(upper_bound):
                mcts_policy = mcts.run(num_simulations=num_simulations, exploration_factor=exploration_factor).unsqueeze(0)
            ...
            if episode % 1 == 0:
                print(f"Episode {episode}, Success Rate: {success_rate:.2f}, Difficulty: {difficulty}")
    ...
    def filter_mcts_policies(self, mcts_policies: list[tuple[torch.Tensor, torch.Tensor]]):
        data = []
...
if __name__ == "__main__":
    ...
    trainer.train(num_episodes=100000, num_simulations=100, exploration_factor=1.0, mcts_cache_size=1000)
