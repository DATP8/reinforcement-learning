    ...
    def search(self, root_state: S, batch_size=64, weight=0.3) -> list[int]:
        ...
        while open_set:
            ...
            for parent_node in nodes_to_expand:
                ...
                for action in self.state_handler.get_possible_actions(
                    parent_node.state
                ):
                    next_state = self.state_handler.get_next_state(
                        parent_node.state, action
                    )
...
if __name__ == "__main__":
    ...
    model.load_state_dict(
        torch.load(
            "models/graph/difficulty43_updates9_iteration13490.pt", map_location=device
        )
    )
    ...
    optimized_circuit = qiskit.transpile(
        routed_circuit, optimization_level=3, coupling_map=CM(topology)
    )
