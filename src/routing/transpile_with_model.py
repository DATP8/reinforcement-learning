...
def plot_heatmap(metric):
    pivot = df.pivot_table(
        index="initial", columns="final_router", values=metric, aggfunc="mean"
    )
...
def plot_fb_heatmaps(df, metric):
    ...
    for i, fb in enumerate(fbs):
        ...
        pivot = sub.pivot_table(index="initial", columns="final_router", values=metric)

        if pivot.empty:
            return

        sns.heatmap(pivot, annot=True, fmt=".2f", cmap="viridis", ax=axes[i])
...
if __name__ == "__main__":
    ...
    for init, fb, final in configs:
        ...
        metrics = collect_metrics(routed, transpile_time)
    ...
    df["pipeline"] = (
        df["initial"] + " → " + df["forward_backward"] + " → " + df["final_router"]
    )
