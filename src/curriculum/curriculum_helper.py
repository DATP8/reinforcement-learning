...
class CircuitMetrics:
    ...
    def get_circuit(self):
        return generate_random_circuit(self.num_qubits, self.difficulty, self.gate_set, self.seed)
    ...
    def from_df(df: pd.DataFrame):
        ...
        for cid, group in df.groupby("id"):
            ...
            circuits.append(CircuitMetrics(
                seed=first.seed,
                difficulty=first.difficulty,
                depth=first.orig_depth,
                num_qubits=first.num_qubits,
                routed=routed,
                gate_set={item.strip() for item in first.gate_set.split(",") if item.strip()}
            ))
