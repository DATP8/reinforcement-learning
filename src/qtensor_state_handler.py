...
class QtensorStateHandler(StateHandler[Qtensor]):
    ...
    def prune(self, state: Qtensor) -> tuple[Qtensor, int]:
        ...
        for i in range(state.gates):
            ...
            if (q1, q2) in self.topology and q1 not in front_layer and q2 not in front_layer:
            ...
            if len(front_layer) >= self.n_qubits-1:
                break
        new_state = new_state[:,removed_gates].cat(torch.zeros((self.n_qubits, layers_removed), dtype=torch.float32), dim=1)
    ...
    def get_random_state(self, difficulty: int):
        ...
        while not flag:
            for _ in range(difficulty):
                ...
                if (q1, q2) not in self.topology or (q2, q1) not in self.topology:
                    flag = True
