...
class MCTS:
    ...
    def iteration(self, current_node: MCTSNode, exploration_factor=1.0) -> float:
        ...
        if current_node.get_visit_count() == 1:
            ...
            current_node.policy = policy[0] #todo: the current implementation only supports batch size of 1
        ...
        new_huristic = self.iteration(child, exploration_factor=exploration_factor) + self.game.get_action_cost(self.get_state(current_node), best_action)
        ...
        new_huristic = min(new_huristic, current_node.huristic) if current_node.huristic is not None else new_huristic
    ...
    def get_ucb_score(self, child: MCTSNode, parent: MCTSNode, exploration_factor=1.0) -> float:
        if parent.policy is None:
            raise ValueError("Parent node policy is not set.")
        
        return self.get_Q(child, parent) - exploration_factor * parent.policy[child.action] * sqrt(parent.get_visit_count()) / (1 + child.get_visit_count())
    ...
    def get_Q(self, child: MCTSNode, parent: MCTSNode) -> float:        
        ...
        return (child.huristic - h_min) / (h_max - h_min + 1e-4)  # Normalize Q value to [0, 1]
    
    @staticmethod
    def get_mcts_policy(node: MCTSNode, temperature=1.0) -> torch.Tensor:
        visit_counts = torch.tensor([child.get_visit_count() for child in node.children.values()], dtype=torch.float32)
