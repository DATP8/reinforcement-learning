...
class TensorState(torch.Tensor):
    ...
    def from_quantum_circuit(qc: QuantumCircuit, horizon=None):
        ...
        for layer in sorted(layers.keys()):
            ...
            layer += 1
            if layer > depth:
                assert horizon is not None, f"Reached depth {layer} which exceeds circuit depth {depth()} without hitting specified horizon {horizon}."
                break
    ...
    def insert_swaps_old(self, qc: QuantumCircuit, path: list, topology: list, game: StateHandler):
        if not path:
            return qc
